...
const mutations = {
	SET_DATA (state, data) {
		if(data.withFilters){
			...
			state.data = state.data.concat(data.articles)
		}
	},
...
const actions = {
	fetchData ({ commit }, params) {
		return axios.get('/api/articles', {params: params}).then ( response => {
			if(response.data.total === 0) { 
				commit('IS_EMPTY', true)
			} else {
				commit('IS_EMPTY', false)
			}
			commit('SET_DATA', {articles: response.data.data, withFilters: params.withFilters })
