...
const mutations = {
	SET_DATA (state, data) {
		if(data.withFilters){
			state.data = data.tutos
		}else{
			state.data = state.data.concat(data.tutos)
		}
	},
...
const actions = {
	fetchData ({ commit }, params) {
		return axios.get('/api/tutos', {params: params}).then ( response => {
			if(response.data.total === 0) { 
				commit('IS_EMPTY', true)
			} else {
				commit('IS_EMPTY', false)
			}
			commit('SET_DATA', {tutos: response.data.data, withFilters: params.withFilters })
